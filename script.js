...
function openDomainDialog() {domainDialog.showModal()};
function closeDomainDialog() {domainDialog.close()};
function alertMessage() {domainAlert.showModal()};
...
// 	chooseBtn.addEventListener('click', () => {domainDialog.showModal()});
// 	closeDomainBtn.addEventListener('click', () => {domainDialog.close()});
// 	closeGameBtn.addEventListener('click', alertMessage);
// 	closeAlertBtn.addEventListener('click', () => {domainAlert.close()});
// }
...
function hasChooseDomain() {
	...
	closeDialogBtn.addEventListener('click', closeInstructionsDialog);
	chooseBtn.addEventListener('click', openDomainDialog);
	closeDomainBtn.addEventListener('click', closeDomainDialog);
	closeGameBtn.addEventListener('click', alertMessage);
  ...
  if (chooseBtn.dataset.domain === biologyInput.value) {
    ...
    chooseDomainQuestions(arrBiology);

  } else if (chooseBtn.dataset.domain === geographyInput.value) {
		wrapperButtons.style.display = 'none';
	  gameDialog.showModal();
    chooseDomainQuestions(arrGeography);

  } else if(chooseBtn.dataset.domain === historyInput.value) {
		wrapperButtons.style.display = 'none';
	  gameDialog.showModal();
    chooseDomainQuestions(arrHistory);

  } else {
    chooseBtn.focus();
    chooseBtn.style.cssText = 'outline: 2px solid red';
    setTimeout(() => {
...
const arrBiology = [
	...
	{
		question: 'You finish the questions! Your brain is enviable!',
...
function chooseDomainQuestions(domain) {
...
function changeQuestion() {
	buttonsChildren.forEach((button) => {
		button.style.background = "rgba(211, 211, 211, 0.39)";
	})
...
function gameOver() {
	if(nr > 10) {
		...
		nextQuestionBtn.removeEventListener('click', changeQuestion);
